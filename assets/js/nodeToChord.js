


function input()
{
    console.clear();

    // inp = document.getElementById("inp").value;
    // patt = [];
    // patt.push(inp[0]);
    // patt.push(inp[1]);
    // patt.push(inp[2]);
    // patt.push(inp[3]);
    // patt.push(inp[4]);
    // patt.push(inp[5]);

   

    notes = PtN(patt);
    lownote = notes[0];
    notes = [...new Set(notes)];
    
    for (let index = 0; index < notes.length; index++) 
    {
        const rotated = RtR(notes, index);
        const basenote = rotated[0];
        const rd = NtR(rotated);
        const tri = DTT(rd);
        const rm = DR(rd, tri);
        const s = DS(basenote, lownote);

        const chord = [basenote, tri, rm, s];

        console.log(chord);
        
    }


}


tuning= ["E", "A", "D", "G", "B", "E"];

const allnotes= ["A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#"];

const tritones = 
{
    maj: [1, 5, 8],
    min: [1, 4, 8],
    sus2: [1, 3, 8],
    sus4: [1, 6, 8],
    '5': [1, 8],
    dim: [1, 4, 7],
    aug: [1, 5, 9],
}

function PtN(pat)   //pattern to notes
{
    nic = [];   //notes in chord
    let i=0;
    while(i < pat.length)
    {
        if(pat[i] != "x")
        {
            stringtoindex = allnotes.findIndex(el => el === tuning[i]);
            temp = parseInt(stringtoindex) + parseInt(pat[i]);
            if(temp > 11)
            {
                temp = temp-12;
            }
            if(temp > 11)
            {
                temp = temp-12;
            }
            nic.push(allnotes[temp]);
        }
        i++;
    }
    return nic;
}


function NtR(notes)     //notes to relatve distance
{
    relativeDistance = [];
    for (let index = 0; index < notes.length; index++)
    {
        relativeDistance.push(allnotes.findIndex(el => el === notes[index]));
    }
    rootoffset = relativeDistance[0]-1;
    for (let index = 0; index < relativeDistance.length; index++)
    {
        relativeDistance[index] = relativeDistance[index] - rootoffset;
        if(relativeDistance[index] < 1)
        {
            relativeDistance[index] = relativeDistance[index] + 12;
        }
    }
    return relativeDistance;
}

function DTT(RelDisc)       //determine tritone
{

    
    return MostSimilar(tritones, RelDisc);
}

function DR(RelDisc, tritone)      //determine remainders
{
    for (let index = 0; index < tritones[tritone].length; index++)       
    {
        if(RelDisc.includes(tritones[tritone][index]))
        {
            const tri = RelDisc.indexOf(tritones[tritone][index]);
            RelDisc.splice(tri, 1);            //getting remainders outside of tritone
        }
    }

    RelDisc.sort(function(a, b){return a - b});

    var StToInt = [[2, "m2"], [3, "M2"], [4, "m3"], [5, "M3"], [6, "4"], [7, "A4/D5"], [8, "5"], [9, "m6"], [10, "M6"], [11, "m7"], [12, "M7"]];

    let remainderInterval = [];
    for (let index = 0; index < RelDisc.length; index++) 
    {
        for (let k = 0; k < StToInt.length; k++)
        {
            if(RelDisc[index] == StToInt[k][0])
            {
                remainderInterval.push(StToInt[k][1]);
            }
        }
    }

    let remainderTone = "Add:";
    for (let index = 0; index < remainderInterval.length; index++)
    {
        remainderTone += remainderInterval[index] + ",";   
    }
    remainderTone = remainderTone.slice(0, -1);

    if(remainderTone.length == 3)
    {
        remainderTone = "";
    }
    return remainderTone;
}

function DS(basenote, lownote)      //determine if slashchords
{
    let slash = "";
    if(lownote != basenote)
    {
        slash = "/" + lownote;
    }
    return slash;
}

/*
function RtC(RelDisc, basenote, lownote)       //relative distance to chord
{
    const tritones = {
    maj: [1, 5, 8],
    min: [1, 4, 8],
    sus2: [1, 3, 8],
    sus4: [1, 6, 8],
    '5': [1, 8],
    dim: [1, 4, 7],
    aug: [1, 5, 9],
    }

    let tritone = MostSimilar(tritones, RelDisc);


    let remainder = RelDisc;
    for (let index = 0; index < tritones[tritone].length; index++)       
    {
        if(remainder.includes(tritones[tritone][index]))
        {
            const tri = remainder.indexOf(tritones[tritone][index]);
            remainder.splice(tri, 1);            //getting remainders outside of tritone
        }
    }

    remainder.sort(function(a, b){return a - b});

    var StToInt = [[2, "m2"], [3, "M2"], [4, "m3"], [5, "M3"], [6, "4"], [7, "A4/D5"], [8, "5"], [9, "m6"], [10, "M6"], [11, "m7"], [12, "M7"]];

    let remainderInterval = [];
    for (let index = 0; index < remainder.length; index++) 
    {
        for (let k = 0; k < StToInt.length; k++)
        {
            if(remainder[index] == StToInt[k][0])
            {
                remainderInterval.push(StToInt[k][1]);
            }
        }
    }

    let remainderTone = "Add:";
    for (let index = 0; index < remainderInterval.length; index++)
    {
        remainderTone += remainderInterval[index] + ",";   
    }
    remainderTone = remainderTone.slice(0, -1);


    if(remainderTone.length == 3)
    {
        remainderTone = "";
    }
     
    let slash = "";
    if(lownote != basenote)
    {
        slash = "/" + lownote;
    }
    
    


    let output = basenote + tritone + remainderTone + slash;

    console.log("Végeredmény: ", output);
    
}
*/

function RtR(notes, index)      //rotate for rootnote
{
    const rotated = notes.slice(index).concat(notes.slice(0, index));
    return rotated;
}



function MostSimilar(reference, compare) {
    let bestKey = null;
    let maxCommon = -1;

    for (const [key, arr] of Object.entries(reference)) {
        const common = arr.filter(x => compare.includes(x)).length;
        if (common > maxCommon) {
            maxCommon = common;
            bestKey = key;
        }
    }
    return bestKey; 
}