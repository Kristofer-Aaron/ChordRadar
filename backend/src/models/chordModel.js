...
export const ChordModel = {
	...
	async findById(id) {
		const [rows] = await pool.query("SELECT * FROM chords WHERE id = ?", [
			id,
		]);
		return rows[0];
	},
	...
	async create(data) {
		...
		if (existing.length > 0) {
			const error = new Error(
				"Chord already exists with this name, tuning, and grip"
			);
	...
	async update(id, data) {
		...
		if (existing.length > 0) {
			const error = new Error(
				"Another chord already exists with this name, tuning, and grip"
			);
			error.status = 409;
			throw error;
		}
